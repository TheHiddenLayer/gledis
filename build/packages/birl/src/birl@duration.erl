...
inner_blur(Values) ->
    _assert_subject = gleam@list:at(Values, 0),
    {ok, Second} = case _assert_subject of
        {ok, _} -> _assert_subject;
        _assert_fail ->
            erlang:error(#{gleam_error => let_assert,
                        message => <<"Assertion pattern match failed"/utf8>>,
    ...
    Leading = gleam@list:at(Values, 1),
    gleam@bool:guard(
        gleam@result:is_error(Leading),
        Second,
        fun() ->
            _assert_subject@1 = gleam@result:values([Leading]),
            [Leading@1] = case _assert_subject@1 of
                [_] -> _assert_subject@1;
                _assert_fail@1 ->
                    erlang:error(#{gleam_error => let_assert,
                                message => <<"Assertion pattern match failed"/utf8>>,
            ...
            _assert_subject@2 = gleam@list:key_find(
                [{year, 31536000000000},
                    {month, 2592000000000},
                    {week, 604800000000},
                    {day, 86400000000},
                    ...
                    {milli_second, 1000},
                    {micro_second, 1}],
                erlang:element(2, Leading@1)
            ),
            {ok, Leading_unit} = case _assert_subject@2 of
            ...
            {ok, Second_unit} = case _assert_subject@3 of
            ...
            Leading@2 = case (erlang:element(1, Second) * Second_unit) < ((Leading_unit
            * 2)
            div 3) of
                true ->
                    Leading@1;
