...
-type message(FTQ) :: {message, FTQ} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-type next(FTR, FTS) :: {continue,
        FTS,
        gleam@option:option(gleam@erlang@process:selector(FTR))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type init_result(FTT, FTU) :: {ready, FTT, gleam@erlang@process:selector(FTU)} |
    {failed, binary()}.

-type self(FTV, FTW) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        FTV,
        gleam@erlang@process:subject(FTW),
        gleam@erlang@process:selector(message(FTW)),
        gleam@otp@system:debug_state(),
        fun((FTW, FTV) -> next(FTW, FTV))}.

-type spec(FTX, FTY) :: {spec,
        fun(() -> init_result(FTX, FTY)),
        integer(),
        fun((FTY, FTX) -> next(FTY, FTX))}.

-type start_error() :: init_timeout |
    {init_failed, gleam@erlang@process:exit_reason()} |
    {init_crashed, gleam@dynamic:dynamic_()}.

-type start_init_message(FTZ) :: {ack,
        {ok, gleam@erlang@process:subject(FTZ)} |
            {error, gleam@erlang@process:exit_reason()}} |
    {mon, gleam@erlang@process:process_down()}.

-spec continue(FUG) -> next(any(), FUG).
continue(State) ->
    {continue, State, none}.

-spec with_selector(next(FUK, FUL), gleam@erlang@process:selector(FUK)) -> next(FUK, FUL).
...
-spec selecting_system_messages(gleam@erlang@process:selector(message(FUW))) -> gleam@erlang@process:selector(message(FUW)).
...
-spec receive_message(self(any(), FUS)) -> message(FUS).
...
-spec init_selector(
    gleam@erlang@process:subject(FZK),
    gleam@erlang@process:selector(FZK)
) -> gleam@erlang@process:selector(message(FZK)).
...
-spec initialise_actor(
    spec(any(), FVN),
    gleam@erlang@process:subject({ok, gleam@erlang@process:subject(FVN)} |
        {error, gleam@erlang@process:exit_reason()})
...
-spec start_spec(spec(any(), FWA)) -> {ok, gleam@erlang@process:subject(FWA)} |
    {error, start_error()}.
start_spec(Spec) ->
...
-spec start(FWG, fun((FWH, FWG) -> next(FWH, FWG))) -> {ok,
        gleam@erlang@process:subject(FWH)} |
    {error, start_error()}.
start(State, Loop) ->
    start_spec(
        {spec,
...
-spec send(gleam@erlang@process:subject(FWN), FWN) -> nil.
send(Subject, Msg) ->
    gleam@erlang@process:send(Subject, Msg).

-spec call(
    gleam@erlang@process:subject(FWP),
    fun((gleam@erlang@process:subject(FWR)) -> FWP),
    integer()
) -> FWR.
