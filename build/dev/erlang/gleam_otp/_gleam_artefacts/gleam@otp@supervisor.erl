...
-type spec(GFL, GFM) :: {spec,
        GFL,
        integer(),
        integer(),
        fun((children(GFL)) -> children(GFM))}.

-opaque children(GFN) :: {ready, starter(GFN)} | {failed, child_start_error()}.

-opaque child_spec(GFO, GFP, GFQ) :: {child_spec,
        fun((GFP) -> {ok, gleam@erlang@process:subject(GFO)} |
            {error, gleam@otp@actor:start_error()}),
        fun((GFP, gleam@erlang@process:subject(GFO)) -> GFQ)}.
...
-spec start_child(child_spec(any(), GFX, GFY), GFX) -> {ok, child(GFY)} |
    {error, child_start_error()}.
...
-spec perform_instruction_for_child(
    GGL,
    instruction(),
    child_spec(any(), GGL, GGN),
    child(GGN)
) -> {ok, {child(GGN), instruction()}} | {error, child_start_error()}.
...
-spec start_and_add_child(starter(GHE), child_spec(any(), GHE, GHH)) -> children(GHH).
...
-spec add(children(GHM), child_spec(any(), GHM, GHP)) -> children(GHP).
...
-spec supervisor(
    fun((GHU) -> {ok, gleam@erlang@process:subject(GHV)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(GHV, GHU, GHU).
supervisor(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-spec worker(
    fun((GIC) -> {ok, gleam@erlang@process:subject(GID)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(GID, GIC, GIC).
worker(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.

-spec returning(
    child_spec(GIK, GIL, any()),
    fun((GIL, gleam@erlang@process:subject(GIK)) -> GIR)
) -> child_spec(GIK, GIL, GIR).
returning(Child, Updater) ->
    {child_spec, erlang:element(2, Child), Updater}.

-spec init(spec(any(), GIW)) -> gleam@otp@actor:init_result(state(GIW), message()).
init(Spec) ->
...
-spec handle_exit(gleam@erlang@process:pid_(), state(GJC)) -> gleam@otp@actor:next(message(), state(GJC)).
...
-spec loop(message(), state(GJH)) -> gleam@otp@actor:next(message(), state(GJH)).
loop(Message, State) ->
    case Message of
        {exit, Exit_message} ->
