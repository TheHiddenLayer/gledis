...
-opaque task(GSH) :: {task,
        gleam@erlang@process:pid_(),
        gleam@erlang@process:pid_(),
        gleam@erlang@process:process_monitor(),
        gleam@erlang@process:selector(message(GSH))}.

-type await_error() :: timeout | {exit, gleam@dynamic:dynamic_()}.

-type message(GSI) :: {from_monitor, gleam@erlang@process:process_down()} |
    {from_subject, GSI}.

-spec async(fun(() -> GSJ)) -> task(GSJ).
async(Work) ->
    Owner = erlang:self(),
...
-spec try_await(task(GSN), integer()) -> {ok, GSN} | {error, await_error()}.
...
-spec await(task(GSR), integer()) -> GSR.
await(Task, Timeout) ->
...
-spec try_await_forever(task(GST)) -> {ok, GST} | {error, await_error()}.
...
-spec await_forever(task(GSX)) -> GSX.
await_forever(Task) ->
