...
-spec create(
    ...
    fun((YO) -> YO),
    fun((YN, YO) -> YN),
    fun((YN, YN) -> gleam@order:order())
) -> fun((YN, YN, YO) -> {ok, gleam@iterator:iterator(YN)} | {error, nil}).
...
-spec create_infinite(
    fun((YS) -> boolean()),
    fun((YS, YT) -> YS),
    fun((YS, YS) -> gleam@order:order())
) -> fun((YS, YT) -> {ok, gleam@iterator:iterator(YS)} | {error, nil}).
