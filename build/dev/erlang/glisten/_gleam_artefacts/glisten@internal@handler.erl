...
-type message(HHL) :: {internal, internal_message()} | {user, HHL}.

-type loop_message(HHM) :: {packet, bitstring()} | {custom, HHM}.

-type loop_state(HHN, HHO) :: {loop_state,
        {ok, {{integer(), integer(), integer(), integer()}, integer()}} |
            {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(HHN)),
        glisten@transport:transport(),
        HHO}.

-type connection(HHP) :: {connection,
        {ok, {{integer(), integer(), integer(), integer()}, integer()}} |
            {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(HHP))}.

-type handler(HHQ, HHR) :: {handler,
        glisten@socket:socket(),
        fun((loop_message(HHQ), HHR, connection(HHQ)) -> gleam@otp@actor:next(loop_message(HHQ), HHR)),
        fun((connection(HHQ)) -> {HHR,
            gleam@option:option(gleam@erlang@process:selector(HHQ))}),
        gleam@option:option(fun((HHR) -> nil)),
        glisten@transport:transport()}.

-spec start(handler(HIB, any())) -> {ok,
        gleam@erlang@process:subject(message(HIB))} |
