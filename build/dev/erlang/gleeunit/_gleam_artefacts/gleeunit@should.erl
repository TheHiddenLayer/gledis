...
-spec equal(GWJ, GWJ) -> nil.
equal(A, B) ->
    gleeunit_ffi:should_equal(A, B).

-spec not_equal(GWK, GWK) -> nil.
not_equal(A, B) ->
    gleeunit_ffi:should_not_equal(A, B).

-spec be_ok({ok, GWL} | {error, any()}) -> GWL.
be_ok(A) ->
    gleeunit_ffi:should_be_ok(A).

-spec be_error({ok, any()} | {error, GWQ}) -> GWQ.
be_error(A) ->
    gleeunit_ffi:should_be_error(A).

-spec be_some(gleam@option:option(GWT)) -> GWT.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
