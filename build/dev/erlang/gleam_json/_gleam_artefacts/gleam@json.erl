...
-spec decode_bits(
    bitstring(),
    fun((gleam@dynamic:dynamic_()) -> {ok, BMI} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, BMI} | {error, decode_error()}.
...
-spec do_decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, BME} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, BME} | {error, decode_error()}.
...
-spec decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, BMA} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, BMA} | {error, decode_error()}.
decode(Json, Decoder) ->
    do_decode(Json, Decoder).
...
-spec nullable(gleam@option:option(BMO), fun((BMO) -> json())) -> json().
...
-spec array(list(BMS), fun((BMS) -> json())) -> json().
