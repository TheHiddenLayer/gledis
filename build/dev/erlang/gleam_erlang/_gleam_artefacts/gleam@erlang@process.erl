...
-opaque subject(FKP) :: {subject, pid_(), gleam@erlang:reference_()} |
    {gleam_phantom, FKP}.

-type do_not_leak() :: any().

-type selector(FKQ) :: any() | {gleam_phantom, FKQ}.
...
-type call_error(FKR) :: {callee_down, gleam@dynamic:dynamic_()} |
    call_timeout |
    {gleam_phantom, FKR}.
...
-spec select(selector(FLI), integer()) -> {ok, FLI} | {error, nil}.
select(From, Within) ->
    gleam_erlang_ffi:select(From, Within).

-spec select_forever(selector(FLM)) -> FLM.
select_forever(From) ->
    gleam_erlang_ffi:select(From).

-spec map_selector(selector(FLO), fun((FLO) -> FLQ)) -> selector(FLQ).
map_selector(A, B) ->
    gleam_erlang_ffi:map_selector(A, B).

-spec merge_selector(selector(FLS), selector(FLS)) -> selector(FLS).
merge_selector(A, B) ->
...
-spec selecting_trapped_exits(selector(FLW), fun((exit_message()) -> FLW)) -> selector(FLW).
...
-spec selecting(selector(FLZ), subject(FMB), fun((FMB) -> FLZ)) -> selector(FLZ).
...
-spec 'receive'(subject(FLC), integer()) -> {ok, FLC} | {error, nil}.
...
-spec selecting_record2(
    selector(FME),
    any(),
    fun((gleam@dynamic:dynamic_()) -> FME)
) -> selector(FME).
...
-spec selecting_record3(
    selector(FMI),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FMI)
) -> selector(FMI).
...
-spec selecting_record4(
    selector(FMM),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FMM)
) -> selector(FMM).
...
-spec selecting_record5(
    selector(FMQ),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FMQ)
) -> selector(FMQ).
...
-spec selecting_record6(
    selector(FMU),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FMU)
) -> selector(FMU).
...
-spec selecting_record7(
    selector(FMY),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FMY)
) -> selector(FMY).
...
-spec selecting_record8(
    selector(FNC),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FNC)
) -> selector(FNC).
...
-spec selecting_anything(selector(FNG), fun((gleam@dynamic:dynamic_()) -> FNG)) -> selector(FNG).
...
-spec try_call(subject(FNR), fun((subject(FNT)) -> FNR), integer()) -> {ok, FNT} |
    {error, call_error(FNT)}.
...
-spec call(subject(FNY), fun((subject(FOA)) -> FNY), integer()) -> FOA.
...
-spec send_after(subject(FOD), integer(), FOD) -> timer().
